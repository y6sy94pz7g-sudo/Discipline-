...
(async () => {
  ...
  await renderPDF(browser, path.join(DIR, 'cover_h.html'), path.join(DIR, 'cover_h.pdf'));
  await renderPDF(browser, path.join(DIR, 'body_h.html'), path.join(DIR, 'body_h.pdf'));
  await browser.close();

  // merge front matter (cover + title + author profile) + body
  const out = await PDFDocument.create();
  let frontCount = 0;
  for (const f of ['cover_h.pdf', 'body_h.pdf']) {
    const src = await PDFDocument.load(fs.readFileSync(path.join(DIR, f)));
    const pages = await out.copyPages(src, src.getPageIndices());
    pages.forEach(p => out.addPage(p));
    if (f === 'cover_h.pdf') frontCount = pages.length; // unnumbered front matter
  }
  ...
  out.setTitle('al-Fatwā al-Ḥamawiyyah');
  out.setAuthor('Shaykhu-l-Islām Ibn Taymiyyah');
  out.setSubject('Überprüft von Abū Muḥammad as-Sanzakī');
  out.setCreator('Abū Muḥammad as-Sanzakī');
  const bytes = await out.save();
  fs.writeFileSync(path.join(DIR, 'Hamawiyyah.pdf'), bytes);
  console.log('Wrote Hamawiyyah.pdf —', out.getPageCount(), 'pages');
